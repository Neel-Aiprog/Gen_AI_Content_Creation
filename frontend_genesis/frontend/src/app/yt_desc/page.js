...
export default function Home() {
  ...
  const [tone_desc, setTone_desc] = useState("casual");
  const [tone_regen, setTone_regen] = useState("casual");
  const [article_desc, setArticle_desc] = useState("");
  ...
  const [error_desc, setError_desc] = useState("");
 ...
 async function handleDescription(e) {
    e.preventDefault();
    if (!topic1.trim()) return setError_desc("Please enter a topic.");
    ...
    try {
      const res = await fetch("/api/youtube-description", {
        ...
        body: JSON.stringify({ topic1, tone_desc }),
      });

      const data = await res.json();
      setArticle_desc(data.description);
    } catch {
      setError_desc("Backend error.");
    } finally {
 ...
 <nav className={styles.navbar}>
      ...
      <div className={styles.contentWrapper}>
        {/* LEFT COLUMN - YOUTUBE DESCRIPTION GENERATOR */}
        <div className={styles.column}>
          <div className={styles.card}>
            <div className={styles.cardHeader}>
              ...
              <p className={styles.cardSubtitle}>Generate Youtube description</p>
            </div>

            <form className={styles.form} onSubmit={handleDescription}>
              <div className={styles.inputGroup}>
                <label className={styles.inputLabel}>Topic</label>
                <textarea 
                  className={styles.textarea} 
                  placeholder="Enter your youtube description topic here..."
                  value={topic1} 
              ...
              <div className={styles.inputGroup}>
                ...
                <select className={styles.select} value={tone_desc} onChange={e => setTone_desc(e.target.value)}>
              ...
              <button 
                className={`${styles.generateButton} ${loading_desc ? styles.loading : ''}`} 
            ...
            {error_desc && <div className={styles.errorMessage}>{error_desc}</div>}
              ...
              <div className={styles.editorBox}>
                ...
                {article_desc && editorLoaded ? (
                  ...
                  // ---------- ADD COPY BUTTON ----------
                    ...
                    onChange={(e, editor) => setArticle_desc(editor.getData())}
                  />
                ) : (
                  <div className={styles.editorPlaceholder}>
                    <div className={styles.placeholderIcon}>✨</div>
                    <p>Your generated youtube description will appear here...</p>
                    <p className={styles.placeholderHint}>Enter a topic and click "Generate Youtube description"</p>
                  </div>
                )}
              </div>
            </div>
